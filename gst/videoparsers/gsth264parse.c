...
gst_h264_parse_process_nal (GstH264Parse * h264parse, GstH264NalUnit * nalu)
{
  ...
  if ((h264parse->format != GST_H264_PARSE_FORMAT_AVC &&
          NAL_TYPE_IS_KEY (nal_type)) || nal_type == GST_H264_NAL_SLICE_IDR) {
...
gst_h264_parse_update_src_caps (GstH264Parse * h264parse, GstCaps * caps)
{
  ...
  if (caps) {
    ...
    src_caps = gst_pad_get_caps (GST_BASE_PARSE_SRC_PAD (h264parse));
    ...
    if (h264parse->idr_pos >= 0) {
      ...
      if (GST_TIME_AS_SECONDS (diff) >= h264parse->interval ||
        ...
        if (h264parse->align == GST_H264_PARSE_ALIGN_NAL) {
            ...
            if ((codec_nal = h264parse->sps_nals[i])) {
              ...
              if (bs) {
                ...
                ok &= gst_byte_writer_put_uint32_be (&bw,
                    (GST_BUFFER_SIZE (codec_nal) << (nls * 8)));
          ...
          /* Replace with a DRM buffer if there is cenc information */
          gst_h264_parse_buffer_add_cenc (h264parse, &new_buf);
