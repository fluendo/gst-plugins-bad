...
static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE ("src",
    ...
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS
    ("audio/x-raw-int, width=16, depth=16, signed=true, endianess=1234,"
     "channels=2, rate=48000;"
     "audio/x-raw-int, width=32, depth=32, signed=true, endianess=1234,"
     "channels=2, rate=48000")
    );
...
#define DEBUG_INIT(bla) \
  GST_DEBUG_CATEGORY_INIT (gst_decklink_audio_src_debug, "decklinksrc", 0, \
      "debug category for decklinksrc element");
GST_BOILERPLATE_FULL (GstDecklinkAudioSrc, gst_decklink_audio_src, GstPushSrc,
    GST_TYPE_PUSH_SRC, DEBUG_INIT);

static void
gst_decklink_audio_src_base_init (gpointer g_class)
{
  ...
  gst_element_class_set_details_simple (element_class, "Decklink Audio Source",
      "Source/Audio", "DeckLink Audio Source", "David Schleef <ds@entropywave.com>");
}
...
gst_decklink_audio_src_init (GstDecklinkAudioSrc * self,
    GstDecklinkAudioSrcClass *klass)
{
...
static GstCaps *
gst_decklink_audio_src_get_caps (GstBaseSrc * bsrc)
{
  GstCaps *caps;

  // We don't support renegotiation
  caps = gst_pad_get_negotiated_caps (GST_BASE_SRC_PAD (bsrc));
